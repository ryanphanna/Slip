...
- **Gemini call timer leak**: every receipt-parsing call to Gemini left its 90-second timeout timer running even after the call finished, instead of clearing it. Found while adding test coverage for the Flash/Pro fallback logic.
...
- **Target item identifiers**: receipt items now retain Target TCIN/item numbers, UPCs, and DPCIs when available for product-level verification.
- **Items catalog coverage**: the Items view now loads the complete user catalog instead of stopping at the first 100 records.
