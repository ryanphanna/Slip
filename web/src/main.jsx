...
function ReceiptDetail({ receipt, onClose, onSaved }) {
  ...
  return <aside className="detail-panel">
    ...
    <div className="detail-body">
      ...
      </form> : <div className="detail-form read-only-detail"><div className="summary-grid"><div><small>Purchased</small><strong>{receipt.date || 'No date'}</strong></div><div><small>Category</small><strong>{receipt.category || 'Other'}</strong></div><div><small>Total</small><strong>{formatMoney(receipt.total, receipt.currency)}</strong></div></div><p className="meta">{receipt.type === 'refund' ? 'Refund' : 'Purchase'}</p><h3>Items</h3><div className="read-only-items">{(receipt.items || []).map((item, index) => <div className="read-only-item" key={`${item.name}-${index}`}><span><strong>{item.publicName || item.name || 'Unnamed item'}</strong>{item.publicName && item.name && item.publicName.trim().toLowerCase() !== item.name.trim().toLowerCase() && <small>{item.name}</small>}</span><span>×{item.quantity || 1}</span><strong>{formatMoney(item.price, receipt.currency)}</strong></div>)}</div></div>}
    </div>
...
function ItemsView({ receipts, catalogItems, onReceiptUpdated }) {
  ...
  return <section className="items-page">
    ...
    {filtered.length === 0 ? <div className="empty"><h2>{mode === 'verified' ? 'No verified items yet.' : 'Nothing needs review.'}</h2><p>{mode === 'verified' ? 'Approve items from the Needs review tab before they appear here.' : 'Newly captured items will appear here for approval.'}</p></div> : <><div className="item-list">{filtered.map((item) => <div className="catalog-item" key={item.id}><div><strong>{item.publicName || item.name || 'Unnamed item'}</strong><small>{item.publicName && item.name && item.publicName.trim().toLowerCase() !== item.name.trim().toLowerCase() ? `${item.name} · ` : ''}{item.merchant} · {item.date}{item.category ? ` · ${item.category}` : ''}</small></div><span className="item-action"><span>×{item.quantity || 1}</span><strong>{formatMoney(item.price)}</strong>{mode === 'review' && <button onClick={() => verifyItem(item)}>Approve</button>}</span></div>)}</div><div className="list-footer"><span className="count">{filtered.length} items</span></div></>}
  </section>;
}
