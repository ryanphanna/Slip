...
function ReceiptDetail({ receipt, onClose, onSaved }) {
  ...
  return <aside className="detail-panel">
    ...
    <div className="detail-body">
      ...
      </form> : <div className="detail-form read-only-detail"><div className="summary-grid"><div><small>Purchased</small><strong>{receipt.date || 'No date'}</strong></div><div><small>Category</small><strong>{receipt.category || 'Other'}</strong></div><div><small>Subtotal</small><strong>{formatMoney(receipt.subtotal, receipt.currency)}</strong></div><div><small>Tax</small><strong>{formatMoney(receipt.tax, receipt.currency)}</strong></div><div><small>Total</small><strong>{formatMoney(receipt.total, receipt.currency)}</strong></div><div><small>Items</small><strong>{(receipt.items || []).length}</strong></div></div><p className="meta">{receipt.type === 'refund' ? 'Refund' : 'Purchase'}</p><h3>Items</h3><div className="read-only-items">{(receipt.items || []).map((item, index) => <div className="read-only-item" key={`${item.name}-${index}`}><span><strong>{item.publicName || item.name || 'Unnamed item'}</strong>{item.publicName && item.name && item.publicName.trim().toLowerCase() !== item.name.trim().toLowerCase() && <small>{item.name}</small>}</span><span>×{item.quantity || 1}</span><strong>{formatMoney(item.price, receipt.currency)}</strong></div>)}</div></div>}
    </div>
