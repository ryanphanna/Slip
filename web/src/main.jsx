...
function ReceiptDetail({ receipt, onClose, onSaved }) {
  ...
  return <aside className="detail-panel">
    ...
    <div className="detail-body">
      ...
      <h3>Items</h3>
      <div className="items">{draft.items.map((item, index) => <div className="item-entry" key={`${item.name}-${index}`}><div className="item-row"><input value={item.name} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, name: e.target.value } : current))} /><input aria-label={`Quantity for ${item.name}`} type="number" min="1" step="1" value={item.quantity ?? 1} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, quantity: Number(e.target.value) } : current))} /><input aria-label={`Price for ${item.name}`} type="number" step="0.01" value={item.price ?? ''} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, price: Number(e.target.value) } : current))} /></div><div className="item-metadata"><input aria-label={`Public name for ${item.name}`} placeholder="Public name" value={item.publicName || ''} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, publicName: e.target.value } : current))} /><input aria-label={`Item number for ${item.name}`} placeholder="Item number" value={item.itemNumber || ''} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, itemNumber: e.target.value } : current))} /><input aria-label={`Product URL for ${item.name}`} placeholder="Product URL" value={item.productUrl || ''} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, productUrl: e.target.value } : current))} /><label className="item-verified"><input aria-label={`Verified ${item.name}`} type="checkbox" checked={item.verified === true} onChange={(e) => update('items', draft.items.map((current, i) => i === index ? { ...current, verified: e.target.checked } : current))} /> Verified</label></div></div>)}</div>
...
function ItemsView({ receipts, catalogItems, onReceiptUpdated }) {
  ...
  return <section className="items-page">
    <div className="items-controls"><div className="item-tabs"><button className={mode === 'verified' ? 'active' : 'secondary'} onClick={() => setMode('verified')}>Verified</button><button className={mode === 'review' ? 'active' : 'secondary'} onClick={() => setMode('review')}>Needs review</button></div><div className="toolbar"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search items or merchants" /><span className="count">{filtered.length} items</span></div></div>
